...
pub fn get_character_episodes_test() {
  ...
  case sisko_episodes {
    [first, second] -> {
      ...
      first.importance |> should.equal(4)
      // contrast value from first episode
      second.season |> should.equal(1)
      second.importance |> should.equal(2)
      // contrast value from second episode
    }
    _ -> should.fail()
  }
  ...
  case bashir_episodes {
    [first, second] -> {
      first.season |> should.equal(1)
      first.importance |> should.equal(0)
      // not in first episode
      second.season |> should.equal(1)
      second.importance |> should.equal(3)
      // in second episode
    }
    _ -> should.fail()
  }
...
pub fn get_organization_episodes_test() {
  ...
  case federation_episodes {
    [first, second] -> {
      first.season |> should.equal(1)
      first.importance |> should.equal(5)
      // in first episode
      second.season |> should.equal(1)
      second.importance |> should.equal(0)
      // not in second episode
    }
    _ -> should.fail()
  }
}
